...
Both repos public on GitHub. `sim` (substrate packages + product app + tools) and `simdocs` (full docs corpus) both ship as public-OSS repositories.
...
- Product app (`apps/web`): MIT for substrate-extracted shapes; proprietary-shaped product code stays MIT as well unless an explicit alternative lands in an ADR amendment.
...
Product app being public also means: secrets never committed (per `book/HARD-RULES.md` single-secrets-root); env shape documented but values stay in operator-local secrets; no secrets in tracked source.
